...
let portalClickSound = new Audio ("./the-portal.mp3");
let spinnerClickSound = new Audio ("./spinner-sound.mp3");
//Crystal clicking
...
spinner.addEventListener("click",()=>{
    ...
    setTimeout(() => {
    spinnerClickSound.pause();
    spinner.style.transition = "0s ease"
    getReward(result)
...
//Shop pop-up
const inventroy= document.querySelector(".inventory");
const invetoryPopup = document.querySelector(".i-popup");
const iXOut = document.querySelector(".x-i-out");                                                                                                                                                  
//open shop
inventroy.addEventListener("click",()=>{
    invetoryPopup.style.visibility = "visible";
    updateInventory();
});
//close shop
iXOut.addEventListener("click",()=>{
    invetoryPopup.style.visibility = "hidden"; 
});
...
function updateInventory() {
    ...
    armorLabel.innerHTML = `Armor Level: ${armorLevel}`;
}
const textBox = document.createElement("p");
multiverseTeleporter.addEventListener("mouseover",(e)=>{
    textBox.innerHTML = dimensionPriceList[currentDimension];
    ...
    switch(true){
        ...
        case (item>= 60 && item <120):
         ...
         case (item>= 120 && item <180):
                crystals -= (crystals * .5)
                break;
          case (item>= 180 && item <300):
                    crystals -= (crystals * .5)
                    break;
            case (item>= 60 && item < 300):
                        crystals -= (crystals * .5);
                        break;
             case (item >= 300):
                           crystals >= 0;
